...
const HospitalDashboard = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
      {/* Header Banner */}
      <div className="glass-panel rounded-3xl p-6 sm:p-8 bg-gradient-to-r from-sky-950/60 via-[#151c2e] to-[#151c2e] border border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-400 text-xs mt-1">License: {profile?.licenseNumber || 'HOSP-VERIFIED-2026'}</p>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
            {profile.inventory.map((item, i) => (
              <div key={i} className="glass-panel p-4 rounded-2xl border border-slate-800 bg-[#151c2e] text-center">
                <span className="text-xs font-bold text-slate-400 block mb-1">{item.bloodGroup}</span>
