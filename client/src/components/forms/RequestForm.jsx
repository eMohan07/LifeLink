...
const RequestForm = ({ onSubmit, initialValues = {}, loading = false }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-lightbg/90 text-slate-900 text-sm pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 focus:border-rose-500 focus:outline-none"
          />
        </div>
      </div>
            ...
            <button
              ...
              className={`py-2 text-xs font-bold uppercase rounded-xl border transition-all ${
                urgency === lvl
                  ...
                  : 'bg-lightbg text-slate-500 border-slate-200 hover:bg-white'
              }`}
            >
              {lvl}
            </button>
      ...
      <div>
        <label className="block text-xs font-semibold text-slate-700 mb-1">Hospital / Delivery Address</label>
        <div className="relative">
      ...
      <div>
        <label className="block text-xs font-semibold text-slate-700 mb-1">Medical Notes (Optional)</label>
        <textarea
          rows={2}
          value={notes}
          onChange={(e) => setNotes(e.target.value)}
          placeholder="Any specific medical requirements..."
          className="w-full bg-lightbg/90 text-slate-900 text-sm px-3 py-2 rounded-xl border border-slate-200 focus:border-rose-500 focus:outline-none resize-none"
        ></textarea>
      </div>
